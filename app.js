...
(function() {
    // 1. 寻找并注入按钮
    function injectResizeBtns() {
        var toolbars = document.querySelectorAll('.rte-tb'); // 找到所有工具栏
        toolbars.forEach(function(tb) {
            if (tb.querySelector('.btn-resize')) return; // 防止重复添加

            // 创建放大按钮
            var btnPlus = document.createElement('button');
            btnPlus.type = 'button';
            btnPlus.className = 'rte-btn btn-resize';
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.btn-resize')) {
            var isPlus = e.target.closest('button').innerHTML.indexOf('plus') > -1;
            
            // 获取当前选中的文字，或者整个编辑器内容
            // 注意：这里针对 makeRTE 的特性，直接操作 DOM 样式
            var selection = window.getSelection();
            if (!selection.rangeCount) return;
            ...
            // 获取当前字体大小 (默认可能是 16px 或 17px)
            var currentSize = parseInt(window.getComputedStyle(node).fontSize);
            if (isNaN(currentSize)) currentSize = 17; 

            // 计算新大小
            var newSize = isPlus ? currentSize + 2 : currentSize - 2;
            if (newSize < 12) newSize = 12; // 最小限制
            if (newSize > 40) newSize = 40; // 最大限制

            // 应用样式 (使用 execCommand 兼容性最好，或者直接改 style)
            // 这里尝试使用 span 包裹来改变局部样式
            document.execCommand('styleWithCSS', false, true);
            document.execCommand('fontSize', false, '7'); // 先设为最大号占位
            
            // 找到刚才生成的 font 标签并修改为精确像素值
            var fonts = document.querySelectorAll('font[size="7"]');
            fonts.forEach(function(f) {
                f.removeAttribute('size');
                f.style.fontSize = newSize + 'px';
                // 清除 font 标签，改用 span，保持代码干净
    ...
    // 延迟执行以确保编辑器加载完毕
    setTimeout(injectResizeBtns, 1000);
})();
